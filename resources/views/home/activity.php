<link href="<?php echo cdn('/css/home-activity.css'); ?>" media="screen" rel="stylesheet" type="text/css" />
<link href="<?php echo cdn('/css/bootstrap.datetimepicker.min.css'); ?>" media="screen" rel="stylesheet" type="text/css" />
<!-- Upcoming Activities -->
    ...
    <?php endif; ?>
            <div class="span6">
                <div id="activity-<?php echo $upcomingActivity->activity_id;?>" class="activity">
                    <div class="image">
                        <img src="<?php echo cdn('/img/activities/activity-'.rand(1, 5).'.gif') ;?>" alt="Activity Image" />
                    </div> <!-- .activity-image -->
                    <div class="detail">
...
<script type="text/javascript">
    $('.btn-primary', '#new-activity').click(function() {
        var activityName = $('#activity-name', '#new-activity').val(),
            startTime    = $('#start-time', '#new-activity').val(),
            endTime      = $('#end-time', '#new-activity').val(),
            place        = $('#place', '#new-activity').val(),
            detail       = $('#detail', '#new-activity').val(),
            csrfToken    = $('meta[name=_token]').attr('content');

        return doCreateActivityAction(activityName, startTime, endTime, place, detail, csrfToken);
    });
</script>
<script type="text/javascript">
    function doCreateActivityAction(activityName, startTime, endTime, place, detail, csrfToken) {
        var postData     = {
        ...
        $('.btn-primary', '#new-activity').html('正在处理...');
        $('.btn-primary', '#new-activity').attr('disabled', 'disabled');
        $.ajax({
            type: 'POST',
            ...
            success: function(result){
                if ( result['isSuccessful'] ) {
                    ...
                    $('.alert-error', '#new-activity').html(errorMessage);
                    $('.alert-error', '#new-activity').removeClass('hide');
                }
                $('.btn-primary', '#new-activity').html('确认');
                $('.btn-primary', '#new-activity').removeAttr('disabled');
            }
        });
    }
</script>
